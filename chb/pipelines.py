...
class ChbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('chb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `chb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM chb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO chb (date, title, link, content)"
